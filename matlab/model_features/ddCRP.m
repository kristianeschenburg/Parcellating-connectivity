...
%   returned.
function [map_z,stats,initial_parc] = ddCRP(features, adj_list, init_c, gt_z, mcmc_passes, ...
...
curr_lp = real(FullProbabilityddCRP_Data(hyp,parcels,features));
...
for pass = 1:mcmc_passes
    ...
    for i = order
        ...
        curr_lp = real(curr_lp + rem_delta_lp + lp(new_neighbor));
        G(i,c(i)) = 1;
        [K, z, parcels] = ConnectedComp(G);
        steps = steps + 1;
    end
